...
class ConsultoriosModelo extends Conexion{
    ...
    public function buscarConsultoriolista($valorBuscado, $nnivel){
        try {
            /* $this->bd->consultaSQL("SELECT consultorios.idplanta, consultorios.idconsultorio, consultorios.cvecons, consultorios.dimensiones, consultorios. disponible, consultorios.estado, propietarios.titulo, propietarios.nombre, propietarios.apellidop, propietarios.apellidom FROM consultorios INNER JOIN propietarios ON consultorios.idconsultorio = propietarios.idconsultorio WHERE consultorios.idplanta = :planta AND propietarios.apellidop LIKE '" . $valorBuscado . "%'"); */ 
    ...
    public function datosRecibosExpedienteConsultorio($dato){
        $estado = 0;
        $this->bd->consultaSQL("SELECT recibos.idrecibo, recibos.idconsultorio, recibos.fechaelaboracion, recibos.fechalimitepago, recibos.cverecibo, recibos.cantidad, recibos.estatus FROM recibos WHERE recibos.idconsultorio = :idconsultorio AND estatus = :estado");            
        $this->bd->enlazarValor(':idconsultorio', $dato);
