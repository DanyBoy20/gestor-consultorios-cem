...
class ReportesModelo extends Conexion{
    ...
    public function adeudoPB(){
        try {
            $this->bd->consultaSQL("SELECT plantas.cveplnt, consultorios.cvecons, propietarios.titulo, propietarios.nombre, propietarios.apellidop, propietarios.apellidom, recibos.fechaelaboracion, recibos.fechalimitepago, recibos.cverecibo, recibos.mensualidad, recibos.cantidad FROM plantas INNER JOIN consultorios ON plantas.idplanta = consultorios.idplanta INNER JOIN propietarios ON consultorios.idconsultorio = propietarios.idconsultorio INNER JOIN recibos ON consultorios.idconsultorio = recibos.idconsultorio  WHERE consultorios.idplanta = :idplanta AND recibos.estatus != 1");
    ...
    public function pagosMes(){
        try {
            $this->bd->consultaSQL("SELECT conceptos. descripcion, conceptos.costo, recargos.idconcepto, recargos.cantidad AS costorecargo, recibos.cverecibo, recibos.cantidad, pagos.cvepago, pagos.fechapago, pagos.comentarios, consultorios.cvecons FROM conceptos INNER JOIN recargos ON conceptos.idconcepto = recargos.idconcepto RIGHT OUTER JOIN recibos ON recargos.idrecibo = recibos.idrecibo INNER JOIN pagos ON recibos.idrecibo = pagos.idrecibo INNER JOIN consultorios ON recibos.idconsultorio = consultorios.idconsultorio WHERE recibos.estatus = 1 AND MONTH(pagos.fechapago) = MONTH(now()) AND YEAR(pagos.fechapago) = YEAR(now())");
            /* $this->bd->enlazarValor(':idplanta', $this->nivel3); */
            $resultados = $this->bd->obtenerConjuntoRegistros1();
            return $resultados;
        } catch (PDOException $e) {
    ...
    public function pagosAnio(){
        try {
            $this->bd->consultaSQL("SELECT conceptos. descripcion, conceptos.costo, recargos.idconcepto, recargos.cantidad AS costorecargo, recibos.cverecibo, recibos.cantidad, pagos.cvepago, pagos.fechapago, pagos.comentarios, consultorios.cvecons FROM conceptos INNER JOIN recargos ON conceptos.idconcepto = recargos.idconcepto RIGHT OUTER JOIN recibos ON recargos.idrecibo = recibos.idrecibo INNER JOIN pagos ON recibos.idrecibo = pagos.idrecibo INNER JOIN consultorios ON recibos.idconsultorio = consultorios.idconsultorio WHERE recibos.estatus = 1 AND YEAR(pagos.fechapago) = YEAR(now())");
            /* $this->bd->enlazarValor(':idplanta', $this->nivel3); */
            $resultados = $this->bd->obtenerConjuntoRegistros1();
            return $resultados;
        } catch (PDOException $e) {
    ...
    public function pagosPorFecha($fecha1, $fecha2){
        try {
            $this->bd->consultaSQL("SELECT conceptos. descripcion, conceptos.costo, recargos.idconcepto, recargos.cantidad AS costorecargo, recibos.cverecibo, recibos.cantidad, pagos.cvepago, pagos.fechapago, pagos.comentarios, consultorios.cvecons FROM conceptos INNER JOIN recargos ON conceptos.idconcepto = recargos.idconcepto RIGHT OUTER JOIN recibos ON recargos.idrecibo = recibos.idrecibo INNER JOIN pagos ON recibos.idrecibo = pagos.idrecibo INNER JOIN consultorios ON recibos.idconsultorio = consultorios.idconsultorio WHERE recibos.estatus = 1 AND pagos.fechapago BETWEEN :fechainicial AND :fechafinal");
