...
class RecibosModelo extends Conexion {
    ...
    public function seleccionarDiaCuatroMes(){
        try {
            // SELECCIONAR 1 DE CADA MES
            $this->bd->consultaSQL("SELECT DATE_SUB(LAST_DAY(NOW()),INTERVAL DAY(LAST_DAY(NOW()))- 6 DAY) AS cuarto_dia");
            $resultados = $this->bd->obtenerRegistro1();
            return $resultados;
        } catch (PDOException $e) {
            /* echo $e->getMessage(); */
    ...
    public function seleccionarDiaDiezMes(){
        try {
            // SELECCIONAR 1 DE CADA MES
            $this->bd->consultaSQL("SELECT DATE_SUB(LAST_DAY(NOW()),INTERVAL DAY(LAST_DAY(NOW()))- 4 DAY) AS dia_diez");
            $resultados = $this->bd->obtenerRegistro1();
            return $resultados;
        } catch (PDOException $e) {
            /* echo $e->getMessage(); */
